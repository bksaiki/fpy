...
@fpy(
    ...
    context=IEEEContext(es=8, nbits=32, rm=RoundingMode.RNE),
)
...
@fpy(
    ...
    context=IEEEContext(es=11, nbits=64, rm=RoundingMode.RNE),
)
...
def arclength_of_a_wiggly_function(n):
    ...
    while i <= n:
        ...
        with IEEEContext(es=15, nbits=80, rm=RoundingMode.RNE) as _:
            t6 = (s1 + s0)
        s1 = t6
        t10 = t2
        with FPCoreContext(precision='integer') as _:
