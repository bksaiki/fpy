...
> **Completed, and kept for the record.**  This is the implementation plan as
> it stood when the work finished, before
> [finiteness-refinement.md](../finiteness-refinement.md) was trimmed to the
> leftovers for `digit-bound`.  The three phases landed on `main` in #311,
> which was squashed -- so the measurements and phase records below are not
> in any branch's history, and this file is the only copy.  Nothing here is
> outstanding work; read it for *why* the analyses ended up as they are.
