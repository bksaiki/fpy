...
Merged (#303).  `SimplifyIf` gained refusal conditions, a `strict` keyword,
`where` / `sites` / `refusals`, and an `EditLog` so cursors forward.
`fpy2.strategies.simplify_if` wraps it.
...
where contexts are concrete, so the only things `strict` still refuses are
out-of-range subscripts and genuine `ASSERT`-overflow contexts.

Take the **default** anyway, for one reason: a guarded subscript is the common
shape, and it is the shape a mask lowers well.  An earlier draft of this
section argued the default from evaluation order — that `tl.where` evaluates
both arms while a lazy consumer would not.  That argument was wrong in both
directions and is retracted: the pass hoists a partial operation into an
unconditional statement before any `IfExpr` sees it, so the `IfExpr` arms are
always plain names and no consumer's laziness recovers anything.
...
## Early returns block the normal form

The one thing here with no route through the existing passes, and the reason
the call constraint above is not simply "run `inline` first".

`SimplifyIf` refuses a `return` inside a branch: control flow escapes, and
there is no expression form for it.  So a function with an early return cannot
be normalized at all.  And `inline` refuses a callee with anything other than
exactly one trailing return, for the same reason from the other side — it
rewrites the trailing return into an assignment, and several returns would
splice early exits into the caller.  So an early-return helper can be neither
inlined nor hoisted, and a branch calling one declines by both routes.

**Measured, on the corpus this backend exists for:** 7 of 20 module-level
functions in `examples/mmasim` have more than one return, including
`dpa_special_values` with four.  `fpy2/libraries` is clean at 0 of 74, so this
is specific to the modelling style the GPU work uses — a cascade of early
returns for NaN, infinity and zero, which is exactly how the reference
algorithms are written.

**Nothing unifies returns.**  `if_bundling` packs mutated variables, not exits;
no other pass touches `ReturnStmt`.  A single-exit normalization —
`if c: return a` ⇝ `if c: r = a else: r = ...; return r` — is item 1 below,
and it is the first thing to build.

**The pipeline inlines everything, which makes the call refusal moot — but
only after this exists.**  `triton.jit` takes `noinline` as an *opt-in*, so
Triton inlines by default and an emitted device call buys nothing at runtime;
FPy forbids recursion, so inlining always terminates.  Inlining is therefore
the right policy, and it leaves `SimplifyIf` no call to refuse and the emitter
no call ABI to define.  The dependency runs one way, though: `inline` refuses a
multi-return callee, so "inline everything" is *gated* on single-exit
normalization rather than an alternative to it.

One trap to know while it does not exist: `inline` with `where=None` *silently
skips* a call it refuses, so the pipeline does not fail there.  It fails later,
at `SimplifyIf`, naming the callee — which reads as a `SimplifyIf` problem when
it is an inlining one.
...
### 1. Single-exit normalization

A pass turning a function with several returns into one with a trailing return,
per *Early returns block the normal form*.  A prerequisite for everything
below: `SimplifyIf` refuses a `return` in a branch, so without this the normal
form declines 7 of 20 mmasim functions outright.

Interpreter-testable, no GPU, and useful beyond this backend — `inline`'s
refusal of a multi-return callee disappears the same way.

### 2. A Triton normal form
...
Settle this before item 3; everything downstream assumes a stable input form.
...
still parallelizes, across the batch. See item 6.
...
### 5. Launcher and differential harness
...
Items 1–5 are a working, testable, torch-callable backend.

### 6. Reductions

`tl.sum` and friends, opt-in per reduction, gated on the precondition item 3
...
Only once item 6 exists and a program appears that should *use* a tensor core.
...
| Item | Sketch |
|---|---|
| 1. Single-exit normalization | 2–3 weeks; no GPU |
| 2. Triton normal form | 2–4 weeks; no GPU; gated on the `Hoistable` question |
| 3. Split and vectorize | 3–5 weeks; no GPU |
| 4. Emitter | 4–6 weeks |
| 5. Launcher + harness | 2–3 weeks; needs a GPU in CI |
| 6. Reductions | unscoped; opt-in |
| 7. `tl.dot` | unscoped |

Items 1–3 are all interpreter-testable, so they parallelize with each other and
need no hardware.
