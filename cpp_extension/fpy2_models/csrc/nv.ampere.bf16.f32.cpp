...
double dpa_special_values__5902d066__c84af79d__b7df414ff(const std::array<float, 8>& A, const std::array<float, 8>& B, double c) {
...
float exponent__5902d066__81d9c83c__b9f1c4018(double x, int8_t emin) {
    auto&& _tmp1 = static_cast<float>(std::logb(x));
    auto&& _tmp2 = static_cast<float>(emin);
    return ((std::isnan(_tmp1) || std::isnan(_tmp2)) ? std::numeric_limits<float>::quiet_NaN() : ((_tmp1 < _tmp2 || (_tmp1 == _tmp2 && std::signbit(_tmp1))) ? _tmp2 : _tmp1));
}

double fused_sum__5902d066__994a64fd__v99d6777c__b526fde49(const std::array<double, 9>& xs, float n) {
...
std::array<double, 9> join__5902d066__1b122f91__bc02ec4c3(const std::array<double, 8>& xs, const std::array<double, 1>& ys) {
    std::array<double, 9> zs = std::array<double, 9>{};
...
float fdpa_round__5902d066__cc1b7ef2__v5af88987__bd185d357(double s) {
...
double t_fdpa__5902d066__c84af79d__bb5951518(const std::array<float, 8>& A, const std::array<float, 8>& B, double c) {
    ...
    if (t24) {
        return dpa_special_values__5902d066__c84af79d__b7df414ff(A, B, c);
    }
    ...
    std::array<std::tuple<double, float, float>, 8> t27 = std::array<std::tuple<double, float, float>, 8>{};
    ...
    for (int8_t t28 = 0; t28 < 8; ++t28) {
        ...
        if ((p_1 == static_cast<double>(0))) {
            ...
            t29 = (exponent__5902d066__6496d95a__b9274069f(a_1, -126) + exponent__5902d066__6496d95a__b9274069f(b_1, -126));
        }
    ...
    if ((c == static_cast<double>(0))) {
        ...
        e_c = exponent__5902d066__81d9c83c__b9f1c4018(c, -126);
    }
    float _tmp6 = es[0];
    ...
    double s = fused_sum__5902d066__994a64fd__v99d6777c__b526fde49(join__5902d066__1b122f91__bc02ec4c3(prods, std::array<double, 1>{{c}}), ((e_max - static_cast<float>(24)) - static_cast<float>(1)));
    return fdpa_round__5902d066__cc1b7ef2__v5af88987__bd185d357(s);
}
...
double t_fdpa_chain(const std::array<float, 16>& A, const std::array<float, 16>& B, float c) {
    ...
    for (int8_t i = 0; i < 16; i += 8) {
        ...
        d = t_fdpa__5902d066__c84af79d__bb5951518(_tmp1, _tmp2, d);
    }
    return d;
}
}
