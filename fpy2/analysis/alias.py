...
class _RegionFacts(DefaultVisitor):
    ...
    def __init__(self, alias: 'AliasAnalysis', def_use: DefineUseAnalysis):
        ...
        # ... and which phi consumed it, so a merge *inside* one object's own
        # class -- a loop head threading a list through its fills -- can be
        # told from one that carries the value somewhere else.
        self._phi_of: dict[Definition, set[Definition]] = {}
        for ps in def_use.phis.values():
            for p in ps:
                for i in (p.lhs, p.rhs):
                    self._phi_of.setdefault(def_use.defs[i], set()).add(p)
        self._def_index = {id(x): i for i, x in enumerate(def_use.defs)}
    ...
    def _same_object(self, d: Definition) -> set[Definition]:
        ...
        ``xs[i] = e`` to what it mutated, so the closure from a definition
        reaches the whole chain that filled one list.
        """
    ...
    def _moves_out(self, cls: set[Definition], e: Var) -> bool:
        ...
        A write-through is not a second read: ``xs[i] = v`` is a use of the
        name, but it makes a definition ``same_object`` with the one it wrote
        through, so it is already in *cls*.  The fill happens before the move.
        """
        for m in cls:
    ...
    def _note_consumed(self, elts: 'tuple[Expr, ...]') -> None:
        ...
        whose value leaves through a phi that is not part of the same object.
        ...
        stores, and all three are one runtime object -- so all three are
        recorded, which is what lets :meth:`referrers_after_moves` drop the
        name (it needs every definition of a name to be consumed).
        """
        if self._repeats:
            return
        for e in elts:
