...
_SPLIT = EditLog(flat.ast, flat_split.ast, (Edit(FuncBody(), 1, 1, 2),))
...
_CONSUMED = EditLog(wrapped.ast, wrapped_after.ast, (Edit(FuncBody(), 0, 1, 2),))
...
_NESTED = EditLog(nested.ast, nested_after.ast, (
    Edit(FuncBody(), 1, 0, 1),               # a pure insertion before the `if`
    Edit(FuncBody().stmt(1).block('ift'), 0, 1, 2),      # the `with` block, in the true arm
))
...
def test_log_rejects_an_edit_past_the_end():
    with pytest.raises(ValueError, match='consumes statements'):
        EditLog(flat.ast, flat_split.ast, (Edit(FuncBody(), 3, 2, 1),))


def test_log_rejects_an_unresolvable_block():
    with pytest.raises(TransformReferenceError):
        EditLog(flat.ast, flat_split.ast, (Edit(FuncBody().stmt(0).block('body'), 0, 1, 1),))


@pytest.mark.parametrize('other', [
    Edit(FuncBody(), 1, 1, 3),                 # the same statement, twice
    Edit(FuncBody(), 1, 0, 1),                 # an insertion into what was consumed
    Edit(FuncBody().stmt(1).block('ift'), 0, 1, 1),        # a block under a consumed statement
])
def test_log_rejects_overlapping_edits(other):
    with pytest.raises(ValueError, match='not disjoint'):
        EditLog(nested.ast, nested_after.ast, (Edit(FuncBody(), 1, 1, 2), other))
