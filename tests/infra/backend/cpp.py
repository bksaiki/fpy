...
def _regression_replaced_slot(
    ...
    overwrites, so `row` still refers to that slot and sees `ys`'s values, as a
    C++ reference does.  `ys` itself is unchanged — the store copied — so the two
    terms distinguish this from a store that rebound the slot."""
    with fp.FP64:
        if len(xss) == 0 or len(ys) == 0:
            return 0
        row = xss[0]
