...
double benchmark_fpy_div(const std::vector<double>& x_vals, 
    ...
    for (size_t i = 0; i < n; i++) {
        result = fpy::div(x_vals[i], y_vals[i], p, rm);
    }
...
double benchmark_mpfr_div(const std::vector<double>& x_vals,
                          const std::vector<double>& y_vals,
                          int p, fpy::RM rm) {
    const size_t n = x_vals.size();
    mpfr_t mx, my, mr;
...
double benchmark_softfloat_div(const std::vector<double>& x_vals,
                              const std::vector<double>& y_vals,
                              fpy::RM rm) {
    const size_t n = x_vals.size();
    ...
    for (size_t i = 0; i < n; i++) {
        // Convert doubles to SoftFloat f32, compute division, convert back
        ...
        float32_t sf_result = f32_div(sf_x, sf_y);
...
int main() {
    ...
    double fpy_time = benchmark_fpy_div(x_vals, y_vals, PRECISION, ROUNDING_MODE);
    std::cout << "Done.\n\n";
    
    // Benchmark MPFR
    std::cout << "Benchmarking MPFR mpfr_div()...\n";
    double mpfr_time = benchmark_mpfr_div(x_vals, y_vals, PRECISION, ROUNDING_MODE);
    std::cout << "Done.\n\n";
    
    // Benchmark SoftFloat
    std::cout << "Benchmarking SoftFloat f32_div()...\n";
    double softfloat_time = benchmark_softfloat_div(x_vals, y_vals, ROUNDING_MODE);
    std::cout << "Done.\n\n";
    ...
    std::cout << "FPY div():               " << fpy_time << " ns/op\n";
